...
from tayloralgebra import TaylorExpansion
...
for K in range(2, 10):
    f = TaylorExpansion([1 / factorial(k) for k in range(K)])
...
x_range = np.linspace(-1, 1, 100)
for K in range(2, 10):
    f = TaylorExpansion([1 / factorial(k) for k in range(K)], cut_off=K)
    g = TaylorExpansion([1, 1, 0.5], cut_off=K)
    h = f(g)
    plt.plot(x_range, h(x_range), label=K)
